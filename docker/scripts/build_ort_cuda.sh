...
cd onnxruntime
./build.sh --parallel --cmake_extra_defines ONNXRUNTIME_VERSION=`cat ./VERSION_NUMBER` CMAKE_HIP_FLAGS=-Wno-deprecated-builtins --config Release --skip_tests --build_wheel --use_migraphx --use_rocm --cuda_version=${ROCM_VERSION} --cuda_home /usr/local/cuda --allow_running_as_root
