...
ENGINE_OPTIONS=("$rocm_options" "$migraphx_options" "$cpu_options" \
				"$torch_options" "$torch2_options" "$torchscript_options" \
				"$tensorflow_options")
...
do
	...
	python3 $testscriptdir/benchmark.py $options -m $model --batch_sizes $batch --sequence_length $sequence -p $precision --result_csv summary.csv --detail.csv detail_csv 1>>${engine}.out 2>>${engine}.err
    done
done <<TESTCONFIG
